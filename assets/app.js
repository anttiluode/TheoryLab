...
async function init(){
  tabSetup();
  try{
    state.manifest=await json('theorylab.json');state.nodes=await json(state.manifest.entrypoints.node_catalog);state.theory=await json(state.manifest.entrypoints.theories[0]);const latestReceipt=state.manifest.entrypoints.receipts[state.manifest.entrypoints.receipts.length-1];state.receipt=await json(latestReceipt);state.history=await json(state.manifest.entrypoints.discovery_history);const kd=await json(state.manifest.entrypoints.genealogy_components);const cd=await json(state.manifest.entrypoints.constraints);const qd=await json(state.manifest.entrypoints.open_questions);state.components=kd.components;state.constraints=cd.constraints;state.questions=qd.questions;
